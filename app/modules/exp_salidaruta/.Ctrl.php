<?php
    class Ctrl {
        ...
        public function tbl_temporal($dt){    
            return gList('exp_salidaruta/components/tbl_temporal',$dt,'tbl_temporal','tbl_temporal_com'); 
        }
        public function tbl_temporal_com($dt){    
            return gList('exp_salidaruta/components/tbl_temporal_com',$dt,'tbl_temporal_com'); 
        }

        public function tbl_detalle($dt){    
            return gList('exp_salidaruta/components/tbl_detalle',$dt,'tbl_detalle','tbl_detalle_com'); 
        }
        ...
        public function del_temp($dt){     
            ...
            if($insert){  ?>
                ...
                vAjax('exp_salidaruta','tbl_temporal_com',0,'temp_com');                      
                </script>
            <?php }else{ ?>
                <script>
        ...
        public function salida_guardar($dt){     
            ...
            if ($ts) { ?>
                <script>                    
                    vAjax('exp_salidaruta','frm_update',<?php echo $ts['@id'] ?>);

                    // vAjax('exp_salidaruta','frm_salidanew',<?php echo $ts['@id'] ?>,'frm_nuevo');
                    // vAjax('exp_salidaruta','tbl_detalle',<?php echo $ts['@id'] ?>,'det'); 
                    aviso('info','Despacho creado correctamente');                                           
                    vAjax('exp_salidaruta','tbl_temporal',0,'temp');                                        
                </script>
            <?php }else{ ?>
                <script>
                    aviso('warning','Falta cargar paquetes / Solicitar un token');
                </script>
                <?php
            }
        }
