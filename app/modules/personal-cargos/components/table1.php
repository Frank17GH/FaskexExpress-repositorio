...
<table class="table table-striped table-bordered table-hover" id="dtable1" width="100%">
    <thead>
        <tr>
            ...
            <th data-hide="esconder" style="width:5px">Ambito</th>
        ...
        <?php 
            if($dt1){
                foreach ($dt1 as $data): 
                    ?>
                        <tr>
                            <td><?php echo str_pad($data[idpersonalcargo], 6, "0", STR_PAD_LEFT) ?></td>
                            <td><?php echo ($data['ca_ambito']=='1')? 'LOCAL':'NACIONAL';?></td>
                            <td><?php echo strtoupper($data[ca_descripcion]); ?></td>
                            <td><?php echo $data[num]; ?> persona</td>
                            <td><?php 
                                    if ($data[ca_estado]=='1') {
                            ...
                            <td style="vertical-align: middle;">
                                <a title="Ver detalles del cliente" class="btn btn-default btn-xs" onclick="vAjax('personal-cargos','mod2',<?php echo $data[idpersonalcargo] ?>,'modal1');">
