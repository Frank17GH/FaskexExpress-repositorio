    ...
    <table class="table">
        ...
        <?php  if($dt1){ $cc=0; foreach ($dt1 as $dta1): $cc++; $total+= $dta1[total]; ?>
            <tr class="encabezado" style="cursor: pointer;"  id="ttrr<?php echo $dta1[iddet] ?>">
                <td>
                    <input type="checkbox" id='chec<?php echo $dta1[iddet] ?>' onclick="sele(<?php echo $dta1[iddet] ?>,<?php echo $cc ?>);">
                </td>
                    <td style="display:none;"><?php echo $dta1[iddet] ?></td>
                <td>
                    <div class="input-group" style="width: 100px;">
                    <input  style="text-align: center;" class="form-control input-xs" placeholder="Entrega" value="<?php echo $dta1[tipo_entrega]; ?>">
                    </div>
                </td>
                <td>
                ...
                <td>
                    <div class="input-group" style="width: 100px;">
                        <div class="icon-addon addon-md"  >
                            <input class="form-control input-xs" id="precio_<?php  echo $cc; ?>" value="<?php echo number_format( $dta1[precio_entrega], 2, '.', ''); ?>" style="text-align: right;" onkeyup="if($('#precio_'+<?php echo $cc; ?>).val()>0){orden_total(<?php echo $cc; ?>);cal();} " disabled>
                            <label style="padding: 4px 0;" class="glyphicon  input-xs"><b class="mon">S/.</b></label>
                        </div> 
                    </div>
                </td>
                ...
                <td >
                    <div style="width: 60px; padding: 0;">
                         <?php if ($dta1[orden]==0) { ?>                      
                    <button type="button" id="btn_guardar_<?php echo $cc; ?>" onclick="vAjax('cotizacion','upd_item',<?php echo $dta1[iddet] ?>+'-/'+$('#ddcant_'+<?php  echo $cc; ?>).val()+'-/'+$('#precio_'+<?php  echo $cc; ?>).val()+'-/'+$('#total_'+<?php  echo $cc; ?>).val());" class="btn btn btn-xs btn-success" disabled="">
                        <i class="fa fa-save fa-1x"></i>
                    </button>
                    <button type="button" id="btn_del_<?php echo $cc; ?>" class="btn btn-xs btn-danger" title="Eliminar"onclick="confir('Eliminacion','¿Seguro que deseas quitar el item seleccionado?','cotizacion','del_det',<?php echo $dta1[iddet]?>+'-/'+<?php echo $dta1[idcot] ?>,'remove');" disabled=""><i class="glyphicon glyphicon-trash"></i> 
                    </button>                    
