    ...
    class Mod extends database{
       ...
       /* public function tabla1($dt){ 
        ...
        public function vDet($Id){      
            return $this->select("SELECT ap.etiqueta, o.fecha_vencimiento, cl.cl_nombres,o.or_producto,
            (SELECT count(dest.iddet) FROM despacho_det dest join apoyo_postal apoy on dest.idapoyo = apoy.idapoyo WHERE apoy.idorden=o.idorden and dest.iddespacho=".$Id.") as cantidad, sum(ap.ap_peso) as peso
                ...
                join clientes cl on c.idclientes = cl.idclientes
                 WHERE dt.iddespacho = ".$Id." and dt.idtipo =1
                group by ap.idorden");
        }

        public function vDet_com($Id){      
            return $this->select("SELECT c.co_nombre_envia,c.co_serie,c.co_fecha,c.co_correlativo,cd.de_descrip,cd.de_peso as peso, 1 as cantidad, cd.de_medida as medida
                FROM `despacho_det` dt
                JOIN comprobante_det cd on dt.idapoyo = cd.iddet
                JOIN comprobante c on cd.idcomprobante = c.idcomprobante
                WHERE dt.iddespacho = ".$Id." and dt.idtipo =2
                ");
        }
         ...
         public function vDetalle($Id){      
            return $this->select("SELECT 
                                ...
                                d.de_tipo,
                                d.de_observacion,
                                (select nombre from ciudad_distrito where iddistrito = d.iddistrito) as distrito,
        ...
        public function tabla1($dt){   
            ...
            return $this->select("SELECT 
                                ...
                                join personal_contratos pc on p.idpersonal = pc.idpersonal
                                join personal_cargo pca on pc.idpersonalcargo = pca.idpersonalcargo
                                WHERE  d.idusuario =".$_SESSION['fasklog']['iduser']
                            );
        ...
        public function cre_despacho($dt){              
            if($dt[mensajero]>0 ){
                return $this->proc("CALL despacho_insert1('".$dt[mensajero]."',".$_SESSION[fasklog][iduser].",'".$dt[ambito]."','".$dt[personal]."','".date('Y-m-d')."','".$dt[hora]."','ninguno',".$_SESSION[fasklog][idgiros].",".$_SESSION[fasklog][local_pre].")");
            }           
        }
        ...
        public function saveDet($dt){
         ...
         public function mod1($dt){
          ...
          public function paquete($dt,$amb){
                ...
                $numero= $fac[2];
                //item=".$numero." and
        return $this->select("SELECT idapoyo,persona,direccion,distrito FROM apoyo_postal a 
