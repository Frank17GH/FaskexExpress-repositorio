<?php 
    foreach ($ts1 as $dta): 
    	$detracc='';
    	if($dta[detraccion]==4){
    		$detracc.='Sujeto a detracción '.$dta[detraccion].'% - Monto Abonar = S/'.number_format($dta[co_total]*0.04,2);
    	}elseif($dta[detraccion]==12){
    		$detracc.='Sujeto a detracción '.$dta[detraccion].'% - Monto Abonar = S/'.number_format($dta[co_total]*0.12,2);
    	}
		...
		<html>
			...
			<body>
				<style>
			       ...
			       #header { position: fixed; left: 0px; top: -120px; right: 0px; height: 150px;  text-align: center; }
			 /*      #footer { position: fixed; left: 0px; bottom: -100px; right: 0px; height: 150px;}
			     ...
			     <?php 
			      ...
			      ?>
				<table style="width: 100%;font-family: Arial; "  id="header" cellspacing="0.7" cellpadding="0.7">
					<tr>
						...
						<td style="text-align: center; vertical-align: middle;">
							...
							<font style="font-size: 9px">
								<?php echo utf8_encode('DIRECCIÓN: ').mb_strtoupper(utf8_encode(__DIREC__)) ?><br><?php echo mb_strtoupper(utf8_encode(__UBICA__)) ?><br>
							</font>
				...
				<table style="width: 100%; font-size: 10px">
					...
					<tr>
						...
						<td colspan="3">: <?php echo mb_strtoupper(utf8_encode($dta[co_direcc_envia])) ?></td>
					</tr>
					<?php 
						if ($dta[co_orden]) {
							?>
								<tr>
						<td style="width: 5px"><?php echo utf8_encode('ORDEN DE COMPRA') ?></td>
						<td colspan="3">: <?php echo $dta[co_orden] ?></td>
					</tr>
							<?php
						}
						if ($dta[co_cargo]) {
				...
				<style type="text/css">th, td {padding: 3px;}</style>
				<table style="width: 100%;border-collapse:collapse; text-align: center; font-size: 10px;margin-top: 10px">
					...
					?>
					<tr>
						<td colspan="7" style="border-bottom:1px solid;"><br></td>
					</tr>
										...
										<table style="width: 100%">
													<?php $subtt=$dta[co_total]/1.18;
														if ($dta[co_tipo]=='03') {
															...
															?>
																<tr>
																	<td colspan="6" style="text-align: right; font-size:12px"><b>OP. GRAVADA : S/</b></td>
																	<td style="text-align: right;font-size:12px"><?php echo number_format($subtt,2) ?></td>
																</tr>
																<tr>
																	<td colspan="6" style="text-align: right; font-size:12px"><b>I.G.V. : S/</b></td>
																	<td style="text-align: right;font-size:12px"><?php echo number_format($dta[co_total]-$subtt,2) ?></td>
																</tr>
																<tr>
																	<td colspan="6" style="text-align: right; font-size:12px;font-weight: bold; "><b>TOTAL A PAGAR : S/</b></td>
																	<td style="text-align: right;font-size:12px"><?php echo number_format($dta[co_total],2) ?></td>
																</tr>
															<?php
														}
													?>
				...
				<table style="width: 100%; font-size: 10px" id="footer">
					...
					<tr>
						<td rowspan="7" style="vertical-align:top;width: calc(100% - 100px)">
							<table style="width: 100%">
								<tr>
									<td >
										<font style="font-size: 11px">
											<?php 
												if ($dta[co_obs]) {
													?>
													<b style="font-weight: bold;"><?php echo utf8_encode('INFORMACIÓN ADICIONAL') ?></b> <br>
													<?php echo utf8_encode($detracc.' / '.$dta[co_obs]) ?><br>
													<?php
												}
											?><br>
											<b style="font-weight: bold;"><?php echo utf8_encode('FORMA DE PAGO: ') ?></b>
													&nbsp;&nbsp;&nbsp;&nbsp;<?php 
														if($dta[co_tipo_pago]==10){
															echo utf8_encode('CRÉDITO 7 DÍAS');
														}elseif($dta[co_tipo_pago]==6){
															echo utf8_encode('CRÉDITO 15 DÍAS');
														}elseif($dta[co_tipo_pago]==7){
															echo utf8_encode('CRÉDITO 30 DÍAS');
														}elseif($dta[co_tipo_pago]==8){
															echo utf8_encode('CRÉDITO 60 DÍAS');
														} elseif($dta[co_tipo_pago]==9){
															echo utf8_encode('CRÉDITO 90 DÍAS');
														}else{
															echo 'CONTADO';
														}
												?><br><br>
											BANCO DE CREDITO DEL PERU Soles 191-2401879-0-02 <br>
											<!-- INTERBANK Soles 2003002698852 <br> -->
											<?php echo utf8_encode('BANCO DE LA NACIÓN') ?> Soles 00-024-058859 
										</font>
									</td>
